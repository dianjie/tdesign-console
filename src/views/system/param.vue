          ...
          <t-row :gutter="[16, 24]">
            <t-col :span="4">
              <t-form-item label="参数名称" name="paramName">
                <t-input v-model="formData.paramName" clearable placeholder="请输入参数名称" />
              </t-form-item>
            </t-col>
            <t-col :span="4">
              <t-form-item label="参数键名" name="paramValue">
                <t-input v-model="formData.paramValue" clearable placeholder="请输入参数键名" />
              </t-form-item>
            </t-col>
          </t-row>
        </t-col>
      ...
      >
        <template #top-content>
          <t-row justify="space-between" style="margin-bottom: 10px">
            <t-col flex="0 0 auto">
              <t-button v-auth="'param_add'" @click="handleAction({ row: {} }, 'add')">新增</t-button>
              <t-button v-auth="'param_delete'" theme="danger" @click="removeCheck">删除</t-button>
            </t-col>
          </t-row>
        </template>
        <template #op="slotProps">
          <a v-auth="'param_view'" class="t-button-link" @click="handleAction(slotProps, 'view')">查看</a>
          <a v-auth="'param_edit'" class="t-button-link" @click="handleAction(slotProps, 'edit')">编辑</a>
          <a v-auth="'param_delete'" class="t-button-link" @click="handleClickDelete(slotProps)">删除</a>
        </template>
      </t-table>
      <t-dialog
...
import DialogForm from '@/viewsBusiness/system/components/param/DialogForm.vue';
import { getList, remove } from '@/api/system/param';
import { COLUMNS } from '@/viewsBusiness/system/constant/param/constant';
