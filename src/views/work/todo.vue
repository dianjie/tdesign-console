    ...
    <div class="table-wrapper">
      ...
      >
        <template #op="slotProps">
          <a v-auth="'work_todo_handle'" class="t-button-link" @click="handleWork(slotProps)">处理</a>
          <a v-auth="'work_todo_detail'" class="t-button-link" @click="handleDetail(slotProps)">详情</a>
          <a v-auth="'work_todo_follow'" class="t-button-link" @click="handleImage(slotProps)">流程图</a>
        </template>
...
import { COLUMNS } from '@/viewsBusiness/work/constant/todo/constant';
...
const { fetchData: oldFetchData, data, pagination, dataLoading } = useRequset(todoList, formData.value, noop);
...
const handleWork = ({ row }) => {
  router.push({
    path: `/work/process/${flowRoute(dictStore.getFlowRoutes, row.category)}/handle/${row.taskId}/${
