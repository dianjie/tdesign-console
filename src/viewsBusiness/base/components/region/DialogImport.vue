<template>
  <t-dialog v-model:visible="formVisible" header="行政区划数据导入" :width="780" :footer="false" @closed="onClosed">
    <template #body>
      <!-- 表单内容 -->
...
import { importRegion, exportTemplate } from '@/api/base/region';
...
const requestMethod = async (param) => {
  try {
    ...
    const data = await importRegion(isCovered, { file: param.raw });
...
const handleTemplate = useDebounceFn(() => {
  exportTemplate().then((res) => {
    downloadByData(res, '行政区划模板.xlsx');
  });
}, 300);
</script>
